    ...
    <div className="min-h-screen bg-black text-white font-sans">
      ...
      <section className="relative py-20 px-6 text-center max-w-4xl mx-auto flex flex-col items-center gap-6">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-neutral-900 border border-neutral-800 text-[10px] uppercase font-bold tracking-widest text-gold">
          <Sparkles className="w-3 h-3 text-gold animate-pulse" /> Premium Ride & Travel Platform
        </div>
      ...
      <section id="fleet" className="py-16 px-6 max-w-6xl mx-auto border-t border-neutral-900">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {vehicles.length === 0 ? (
            ...
            vehicles.map((v) => (
              <div key={v.id} className="card-luxury p-6 flex items-center justify-between gap-4 transition-all duration-300 hover:border-gold/30 hover:-translate-y-1.5">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-neutral-950 flex items-center justify-center border border-gold/15 shadow-[0_4px_12px_rgba(0,0,0,0.5)]">
                    {(() => {
                      const Icon = VEHICLE_ICONS[v.type?.toLowerCase()] || Car;
                      return <Icon className="w-5 h-5 text-gold" />;
                    })()}
                  </div>
                  <div>
                    <h3 className="font-bold text-white text-sm capitalize font-serif tracking-wide">{v.brand} {v.model}</h3>
                    <p className="text-[#D4AF37] text-[9px] uppercase font-bold tracking-widest mt-0.5">{v.type}</p>
                    <p className="text-muted text-xs mt-1.5">Capacity: <span className="text-white font-semibold">{v.capacity} Pax</span></p>
                  </div>
                </div>
                <button
                  onClick={() => handleBookAction('/book/city')}
                  className="px-4 py-2 bg-transparent text-gold border border-gold/30 hover:bg-gold hover:text-black hover:border-gold rounded-full text-[10px] uppercase tracking-widest font-black transition-all duration-300 active:scale-95"
                >
                  Book
                </button>
              </div>
      ...
      <section id="packages" className="py-16 px-6 max-w-6xl mx-auto border-t border-neutral-900">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {packages.length === 0 ? (
            ...
            packages.map((p) => (
              <div key={p.id} className="card-luxury p-6 flex flex-col justify-between gap-5 relative transition duration-300 hover:-translate-y-2">
                <div className="flex flex-col gap-3">
                  <div className="flex justify-between items-start">
                    <h3 className="font-bold text-white text-lg truncate pr-2 font-serif tracking-wide">{p.name}</h3>
                    <span className="text-xs font-black text-[#D4AF37] bg-black/60 px-3 py-1.5 rounded-xl border border-gold/25 shadow-[0_4px_12px_rgba(212,175,55,0.08)]">${Number(p.price).toFixed(2)}</span>
                  </div>
                  <p className="text-[#A3A3A3] text-xs leading-relaxed mt-1 line-clamp-3 font-light">{p.description}</p>
                </div>
                
                <div className="border-t border-neutral-900/60 pt-3 flex items-center justify-between text-[10px] text-neutral-400">
                  ...
                  <span className="flex items-center gap-1.5">
                    ...
                    Up to {p.max_persons} Pax
                  </span>
                </div>

                <button
                  onClick={() => handleBookAction('/book/package')}
                  className="btn-premium w-full py-3 rounded-full font-bold text-xs mt-2"
                >
                  Book Tour Package
                </button>
              </div>
