...
export default function ActivateAccount() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/auth/activate-driver', { token, password });
      setSuccess(true);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-[#050505] px-4 font-sans select-none overflow-hidden relative">
      ...
      <div 
        ...
        role="main"
      >
        {/* Subtle Decorative Golden Glow line */}
        ...
        {success ? (
          <div className="flex flex-col gap-5 text-center relative z-10">
            ...
            <div className="flex flex-col gap-2">
              <p className="text-white text-sm font-semibold">Account Activated Successfully</p>
              <p className="text-[#9CA3AF] text-xs leading-relaxed font-light">
                Your driver account is now active and your password has been securely set. You can now sign in using your new credentials.
              </p>
            </div>
            <Link 
              to="/login"
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4 relative z-10">
            <p className="text-xs text-[#9CA3AF] leading-relaxed font-light mb-1">
              Please choose a secure password to activate your TaxiTrio driver account. Minimum 6 characters.
            </p>
